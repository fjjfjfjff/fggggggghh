...
TEXTS = {
    "ru": {
        ...
        "added": f'{ce("check")} Добавлен',
            ...
            f'https://t.me/{"{bot_username}"}?start={"{deal_id}"}\n\n'
            f'Скопируйте ссылку и отправьте продавцу.'
        ),
        "deal_card_seller": (
            f'{ce("card")} Информация о сделке #{"{deal_id}"}\n\n'
            f'{ce("wave")} Вы продавец в сделке.\n'
            f'{ce("letter")} Покупатель: @{"{buyer_username}"}\n\n'
            f'{ce("scroll")} Вы продаёте: {"{description}"}\n\n'
            f'{ce("cart")} Товар: {"{deal_type}"}\n\n'
            f'{ce("briefcase")} Способ оплаты: {"{currency_label}"}\n\n'
            f'ID сделки: {"{deal_id}"}\n\n'
            f'Сумма к получению: {"{amount_after}"} {"{currency_label}"}\n\n'
            f'Ожидайте подтверждения оплаты от покупателя.\n'
            f'Сохраните ID сделки!\n\n'
            f'В случае проблем обратитесь в поддержку — {ESCROW_URL}'
        ),
        "seller_joined_deal": (
            f'{ce("wave")} Пользователь @{"{username}"}\n'
            f'Присоединился к сделке #{"{deal_id}"} как продавец\n\n'
            f'{ce("folder")} Успешных сделок продавца: {"{deals_count}"}\n'
            f'{ce("check")} Проверенный пользователь\n\n'
            f'Проверьте соответствие пользователя'
        ),
        "seller_joined_buyer_notify": (
            f'{ce("check")} Продавец @{"{seller_username}"} присоединился к сделке #{"{deal_id}"}!\n\n'
            f'{ce("cash")} Сумма к оплате: {"{amount}"} {"{currency_label}"}\n\n'
            f'Теперь вы можете подтвердить оплату.'
        ),
                "withdraw_choose": (
            f'<b>ВЫВОД СРЕДСТВ</b>\n\n'
            f'<b>Ваши балансы:</b>\n'
            f'{ce("cash")} {{balance_rub:.2f}} RUB\n'
            f'{ce("ton")} {{balance_ton:.4f}} TON\n'
            f'{{balance_stars:.2f}} Stars\n\n'
            f'Выберите способ вывода:'
        ),
        "withdraw_locked": (
            f'<b>ВЫВОД {{label}}</b>\n\n'
            f'{ce("briefcase")} Ваши успешные сделки: <b>{{deals_done}}</b>\n'
            f'{ce("folder")} Требуется: {{min_deals}}\n'
            f'Осталось: <b>{{remaining}}</b>\n\n'
            f'Вывод недоступен. Проведите ещё сделок.'
        ),
        "withdraw_available": (
            f'<b>ВЫВОД {{label}}</b>\n\n'
            f'Доступно к выводу: <b>{{balance}} {{currency}}</b>\n\n'
            f'Для вывода обратитесь в поддержку — @NotCoinSafety'
        ),
        "btn_withdraw_card": "На карту / по номеру",
        "btn_withdraw_ton": "На TON-кошелек",
        "btn_withdraw_stars": "На Stars",
        "withdraw_label_card": "RUB",
        "withdraw_label_ton": "TON",
        "withdraw_label_stars": "Stars",
    ...
    "en": {
        ...
        "added": f'{ce("check")} Added',
            ...
            f'Seller link:\n'
            f'https://t.me/{"{bot_username}"}?start={"{deal_id}"}\n\n'
            f'Copy the link and send it to the seller.'
        ),
        "deal_card_seller": (
            f'🪪 Deal info #{"{deal_id}"}\n\n'
            f'👋 You are the seller in this deal.\n'
            f'✉️ Buyer: @{"{buyer_username}"}\n\n'
            f'📜 You are selling: {"{description}"}\n\n'
            f'🛒 Item: {"{deal_type}"}\n\n'
            f'💼 Payment method: {"{currency_label}"}\n\n'
            f'Deal ID: {"{deal_id}"}\n\n'
            f'Amount to receive: {"{amount_after}"} {"{currency_label}"}\n\n'
            f'Wait for payment confirmation from the buyer.\n'
            f'Save the deal ID!\n\n'
            f'In case of problems contact support — {ESCROW_URL}'
        ),
        "seller_joined_deal": (
            f'👋 User @{"{username}"}\n'
            f'Joined deal #{"{deal_id}"} as seller\n\n'
            f'📁 Seller successful deals: {"{deals_count}"}\n'
            f'✅ Verified user\n\n'
            f'Check the user match'
        ),
        "seller_joined_buyer_notify": (
            f'✅ Seller @{"{seller_username}"} joined deal #{"{deal_id}"}!\n\n'
            f'💰 Amount to pay: {"{amount}"} {"{currency_label}"}\n\n'
            f'You can now confirm payment.'
        ),
                "withdraw_choose": (
        ...
        "withdraw_locked": (
            f'<b>WITHDRAW {{label}}</b>\n\n'
            f'{ce("briefcase")} Your successful deals: <b>{{deals_done}}</b>\n'
            f'{ce("folder")} Required: {{min_deals}}\n'
            f'Remaining: <b>{{remaining}}</b>\n\n'
            f'Withdrawal unavailable. Complete more deals.'
        ),
        "withdraw_available": (
            f'<b>WITHDRAW {{label}}</b>\n\n'
            f'Available: <b>{{balance}} {{currency}}</b>\n\n'
            f'To withdraw contact support — @NotCoinSafety'
        ),
        "btn_withdraw_card": "To card / by number",
        "btn_withdraw_ton": "To TON wallet",
        "btn_withdraw_stars": "To Stars",
        "withdraw_label_card": "RUB",
        "withdraw_label_ton": "TON",
        "withdraw_label_stars": "Stars",
    ...
    "ar": {
        ...
        "added": f'{ce("check")} مضاف',
            ...
            f'المبلغ: {"{amount}"} {"{currency_label}"}\n'
            f'✨ العملة: {"{currency_label}"}\n'
            f'🛒 المنتج: {"{deal_type}"}\n'
            f'📜 الوصف: {"{description}"}\n'
            f'رابط البائع:\n'
            f'https://t.me/{"{bot_username}"}?start={"{deal_id}"}\n\n'
            f'انسخ الرابط وأرسله للبائع.'
        ),
        "deal_card_seller": (
            f'🪪 معلومات الصفقة #{"{deal_id}"}\n\n'
            f'👋 أنت البائع في هذه الصفقة.\n'
            f'✉️ المشتري: @{"{buyer_username}"}\n\n'
            f'📜 أنت تبيع: {"{description}"}\n\n'
            f'🛒 المنتج: {"{deal_type}"}\n\n'
            f'💼 طريقة الدفع: {"{currency_label}"}\n\n'
            f'معرف الصفقة: {"{deal_id}"}\n\n'
            f'المبلغ المستلم: {"{amount_after}"} {"{currency_label}"}\n\n'
            f'انتظر تأكيد الدفع من المشتري.\n'
            f'احفظ معرف الصفقة!\n\n'
            f'في حالة المشاكل تواصل مع الدعم — {ESCROW_URL}'
        ),
        "seller_joined_deal": (
            f'👋 المستخدم @{"{username}"}\n'
            f'انضم للصفقة #{"{deal_id}"} كبائع\n\n'
            f'📁 صفقات البائع الناجحة: {"{deals_count}"}\n'
            f'✅ مستخدم موثق\n\n'
            f'تحقق من مطابقة المستخدم'
        ),
        "seller_joined_buyer_notify": (
            f'✅ البائع @{"{seller_username}"} انضم للصفقة #{"{deal_id}"}!\n\n'
            f'💰 المبلغ للدفع: {"{amount}"} {"{currency_label}"}\n\n'
            f'يمكنك الآن تأكيد الدفع.'
        ),
                "withdraw_choose": (
            f'<b>سحب الأموال</b>\n\n'
            f'<b>أرصدتك:</b>\n'
        ...
        "withdraw_locked": (
            f'<b>سحب {{label}}</b>\n\n'
            f'{ce("briefcase")} صفقاتك الناجحة: <b>{{deals_done}}</b>\n'
            f'{ce("folder")} المطلوب: {{min_deals}}\n'
            f'المتبقي: <b>{{remaining}}</b>\n\n'
            f'السحب غير متاح. أتمم المزيد من الصفقات.'
        ),
        "withdraw_available": (
            f'<b>سحب {{label}}</b>\n\n'
            f'المتاح للسحب: <b>{{balance}} {{currency}}</b>\n\n'
            f'للسحب تواصل مع الدعم — @NotCoinSafety'
        ),
        "btn_withdraw_card": "إلى بطاقة / رقم",
        "btn_withdraw_ton": "إلى محفظة TON",
        "btn_withdraw_stars": "إلى Stars",
        "withdraw_label_card": "RUB",
        "withdraw_label_ton": "TON",
        "withdraw_label_stars": "Stars",
    ...
    "zh": {
        ...
        "deal_card_seller": (
            f'🪪 交易信息 #{"{deal_id}"}\n\n'
            f'👋 您是此交易的卖家。\n'
            f'✉️ 买家: @{"{buyer_username}"}\n\n'
            f'📜 您出售: {"{description}"}\n\n'
            f'🛒 商品: {"{deal_type}"}\n\n'
            f'💼 支付方式: {"{currency_label}"}\n\n'
            f'交易ID: {"{deal_id}"}\n\n'
            f'收款金额: {"{amount_after}"} {"{currency_label}"}\n\n'
            f'等待买家确认付款。\n'
            f'请保存交易ID！\n\n'
            f'如有问题请联系支持 — {ESCROW_URL}'
        ),
        "seller_joined_deal": (
            f'👋 用户 @{"{username}"}\n'
            f'作为卖家加入了交易 #{"{deal_id}"}\n\n'
            f'📁 卖家成功交易数: {"{deals_count}"}\n'
            f'✅ 已验证用户\n\n'
            f'请核实用户信息'
        ),
        "seller_joined_buyer_notify": (
            f'✅ 卖家 @{"{seller_username}"} 加入了交易 #{"{deal_id}"}！\n\n'
            f'💰 待付金额: {"{amount}"} {"{currency_label}"}\n\n'
            f'您现在可以确认付款。'
        ),
                "withdraw_choose": (
            f'<b>提款</b>\n\n'
        ...
        "withdraw_locked": (
            f'<b>提款 {{label}}</b>\n\n'
            f'{ce("briefcase")} 您的成功交易: <b>{{deals_done}}</b>\n'
            f'{ce("folder")} 需要: {{min_deals}}\n'
            f'剩余: <b>{{remaining}}</b>\n\n'
            f'提款不可用。请完成更多交易。'
        ),
        "withdraw_available": (
            f'<b>提款 {{label}}</b>\n\n'
            f'可提金额: <b>{{balance}} {{currency}}</b>\n\n'
            f'提款请联系支持 — @NotCoinSafety'
        ),
        "btn_withdraw_card": "到银行卡/电话",
        "btn_withdraw_ton": "到TON钱包",
        "btn_withdraw_stars": "到Stars",
        "withdraw_label_card": "RUB",
        "withdraw_label_ton": "TON",
        "withdraw_label_stars": "Stars",
