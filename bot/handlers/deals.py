...
async def enter_amount(message: Message, session: AsyncSession, state: FSMContext):
    ...
    if amount is None:
        await message.answer(get_text(user.language, "err_invalid_amount"), reply_markup=cancel_to_menu_kb(lang=user.language))
        return
...
async def show_deal_card(message: Message, session: AsyncSession, deal_id: str, user):
    ...
    if not deal:
        await message.answer(get_text(user.language, "err_deal_not_found"), reply_markup=back_to_menu_red_kb(lang=user.language))
        return
    ...
    if deal.status not in (DealStatus.PENDING, DealStatus.ACTIVE):
        await message.answer(get_text(user.language, "err_deal_closed"), reply_markup=back_to_menu_red_kb(lang=user.language))
        return

    updated_deal = await set_deal_buyer(session, deal_id, user.telegram_id)
    if not updated_deal:
        await message.answer(get_text(user.language, "err_join_failed"), reply_markup=back_to_menu_red_kb(lang=user.language))
        return
