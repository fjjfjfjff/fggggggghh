...
async def enter_amount(message: Message, session: AsyncSession, state: FSMContext):
    ...
    if amount is None:
        await message.answer("Введите корректную сумму, например: 100.5", reply_markup=cancel_to_menu_kb(lang=user.language))
        return
...
async def show_deal_card(message: Message, session: AsyncSession, deal_id: str, user):
    ...
    if not deal:
        await message.answer("Сделка не найдена.", reply_markup=back_to_menu_red_kb(lang=user.language))
        return
    ...
    if deal.status not in (DealStatus.PENDING, DealStatus.ACTIVE):
        await message.answer("Эта сделка уже завершена или отменена.", reply_markup=back_to_menu_red_kb(lang=user.language))
        return

    updated_deal = await set_deal_buyer(session, deal_id, user.telegram_id)
    if not updated_deal:
        await message.answer("Не удалось присоединиться к сделке.", reply_markup=back_to_menu_red_kb(lang=user.language))
        return
