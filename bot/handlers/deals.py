...
from bot.database.queries import (
    ...
    set_deal_buyer, set_deal_seller, update_deal_status, complete_deal, get_user,
)
...
from bot.keyboards.main_menu import (
    ...
    seller_notify_kb, back_to_menu_red_kb, confirm_receive_kb,
)
...
async def enter_description(message: Message, session: AsyncSession, state: FSMContext):
    ...
    if role == "seller":
        # Продавец создаёт — ссылка для покупателя
        ...
        # Покупатель создаёт — ссылка для продавца
        deal = await create_deal(
            session=session,
            seller_id=None,
        ...
        await state.clear()
        text = get_text(
            user.language, "deal_created_buyer",
            amount=amount,
            currency_label=currency_label,
            deal_type=deal_type_label,
            description=description,
...
async def show_deal_card(message: Message, session: AsyncSession, deal_id: str, user):
    """Обрабатывает переход по ссылке — определяет роль по тому кто уже есть в сделке."""
    ...
    currency_label = CURRENCY_LABELS.get(
        (deal.payment_method if isinstance(deal.payment_method, str) else deal.payment_method.value).lower(),
        "TON"
    )
    deal_type_label = get_deal_type_label(
        user.language,
        (deal.deal_type if isinstance(deal.deal_type, str) else deal.deal_type.value).lower()
    )
    ...
    # Если в сделке нет продавца — значит создал покупатель, присоединяемся как продавец
    if deal.seller_id is None:
        if deal.buyer_id == user.telegram_id:
            await message.answer(
        ...
        updated_deal = await set_deal_seller(session, deal_id, user.telegram_id)
        if not updated_deal:
            await message.answer(
        ...
        buyer_username = buyer.username if buyer and buyer.username else str(deal.buyer_id)

        # Продавцу показываем карточку сделки
        text = get_text(
            user.language, "deal_card_seller",
            deal_id=deal.deal_id,
        ...
        # Покупателю — уведомление что продавец присоединился + кнопка подтвердить оплату
        if buyer:
            buyer_lang = buyer.language if buyer else "ru"
            notify_text = get_text(
                buyer_lang, "seller_joined_buyer_notify",
            ...
            try:
                await message.bot.send_animation(
                    ...
                    caption=notify_text,
                    parse_mode="HTML",
                    reply_markup=deal_buyer_kb(deal.deal_id, lang=buyer_lang),
                )
            except Exception:
                pass

            # Уведомление о присоединении продавца (для истории)
            joined_text = get_text(
                buyer_lang, "seller_joined_deal",
        ...
        # Продавец уже есть — значит создал продавец, присоединяемся как покупатель
        ...
        # Продавцу — уведомление что покупатель присоединился
        if seller:
