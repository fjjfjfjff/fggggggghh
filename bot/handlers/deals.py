...
from bot.config import settings, GIF_FILE_ID, DEAL_TYPES, PAYMENT_METHODS
from bot.database.queries import (
    get_or_create_user, get_deal, create_deal,
    set_deal_buyer, update_deal_status, complete_deal,
)
...
async def create_deal_start(callback: CallbackQuery, session: AsyncSession, state: FSMContext):
    ...
    await edit_or_send(callback, text, choose_role_kb())
...
async def choose_role(callback: CallbackQuery, session: AsyncSession, state: FSMContext):
    ...
    await edit_or_send(callback, text, choose_type_kb())
    await callback.answer()
...
async def choose_type(callback: CallbackQuery, session: AsyncSession, state: FSMContext):
    ...
    user = await get_or_create_user(session, callback.from_user.id, callback.from_user.username)
    deal_type_label = DEAL_TYPES.get(deal_type, deal_type)
    text = get_text(user.language, "choose_payment", deal_type=deal_type_label)
    await edit_or_send(callback, text, choose_payment_kb())
    await callback.answer()
...
async def choose_payment(callback: CallbackQuery, session: AsyncSession, state: FSMContext):
    ...
    await edit_or_send(callback, text, cancel_to_menu_kb())
    await callback.answer()
...
async def enter_amount(message: Message, session: AsyncSession, state: FSMContext):
    ...
    if amount is None:
        await message.answer("Введите корректную сумму, например: 100.5", reply_markup=cancel_to_menu_kb())
        return
...
async def enter_description(message: Message, session: AsyncSession, state: FSMContext):
    ...
    if not validate_description(description, deal_type):
        await message.answer(
            "Некорректный формат. Проверьте данные и попробуйте снова.",
            reply_markup=cancel_to_menu_kb(),
        )
        return
...
async def show_deal_card(message: Message, session: AsyncSession, deal_id: str, user):
    ...
    if not deal:
        await message.answer("Сделка не найдена.", reply_markup=back_to_menu_red_kb())
        return

    if deal.seller_id == user.telegram_id:
        await message.answer(
            "Вы являетесь продавцом этой сделки. Отправьте ссылку покупателю.",
            reply_markup=back_to_menu_red_kb(),
        )
        return

    if deal.status not in (DealStatus.PENDING, DealStatus.ACTIVE):
        await message.answer("Эта сделка уже завершена или отменена.", reply_markup=back_to_menu_red_kb())
        return

    updated_deal = await set_deal_buyer(session, deal_id, user.telegram_id)
    if not updated_deal:
        await message.answer("Не удалось присоединиться к сделке.", reply_markup=back_to_menu_red_kb())
        return
    ...
    deal_type_label = DEAL_TYPES.get(deal.deal_type if isinstance(deal.deal_type, str) else deal.deal_type.value, "")
    payment_label = PAYMENT_METHODS.get(deal.payment_method if isinstance(deal.payment_method, str) else deal.payment_method.value, "")
    ...
    await message.answer_animation(
        ...
        caption=text,
        parse_mode="HTML",
        reply_markup=deal_buyer_kb(deal.deal_id),
    )
...
async def confirm_payment(callback: CallbackQuery, session: AsyncSession):
    ...
    if not deal or deal.buyer_id != user.telegram_id:
        await callback.answer("Вы не являетесь покупателем этой сделки.", show_alert=True)
        return

    if deal.status != DealStatus.ACTIVE:
        await callback.answer("Невозможно подтвердить оплату для этой сделки.", show_alert=True)
        return

    await update_deal_status(session, deal_id, DealStatus.PAID)

    text_buyer = get_text(user.language, "payment_confirmed_buyer")
    await edit_or_send(callback, text_buyer, back_to_menu_red_kb())
    ...
    try:
        await callback.bot.send_animation(
            ...
            caption=notify_text,
            parse_mode="HTML",
            reply_markup=seller_notify_kb(deal.deal_id),
        )
    except Exception:
        pass
...
async def confirm_transfer(callback: CallbackQuery, session: AsyncSession):
    ...
    if not deal or deal.seller_id != user.telegram_id:
        await callback.answer("Вы не являетесь продавцом этой сделки.", show_alert=True)
        return

    if deal.status != DealStatus.PAID:
        await callback.answer("Невозможно завершить сделку на данном этапе.", show_alert=True)
        return

    completed = await complete_deal(session, deal_id)
    ...
    text = get_text(
        user.language, "deal_completed_seller",
        amount_after=deal.amount_after_commission,
        currency_label=currency_label,
        deal_id=deal.deal_id,
    )

    await edit_or_send(callback, text, back_to_menu_red_kb())
    await callback.answer()
