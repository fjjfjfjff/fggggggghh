...
async def save_ton_wallet(message: Message, session: AsyncSession, state: FSMContext):
    ...
    if len(wallet) < 10:
        await message.answer(get_text(user.language, "err_invalid_wallet"), reply_markup=back_to_menu_red_kb(lang=user.language))
        return
...
async def save_card(message: Message, session: AsyncSession, state: FSMContext):
    ...
    if len(card) < 10:
        await message.answer(get_text(user.language, "err_invalid_card"), reply_markup=back_to_menu_red_kb(lang=user.language))
        return
