...
async def get_or_create_user_with_flag(session: AsyncSession, telegram_id: int, username: str | None = None):
...
async def send_language_choice(target):
    """Показывает выбор языка новому пользователю."""
    text = "Выберите язык / Choose language / اختر اللغة:"
    msg = target.message if isinstance(target, CallbackQuery) else target
    await msg.answer_animation(
        animation=GIF_FILE_ID,
        caption=text,
...
async def cmd_start(message: Message, command: CommandObject, session: AsyncSession, state: FSMContext):
    ...
    user, is_new = await get_or_create_user_with_flag(session, message.from_user.id, message.from_user.username)

    if args.startswith("ref_"):
        referral_code = args[4:]
        from bot.database.queries import get_user_by_referral
    ...
    # Новым пользователям показываем выбор языка, старым — главное меню
    if is_new:
...
async def set_language(callback: CallbackQuery, session: AsyncSession):
    ...
    from bot.database.queries import update_user_language, get_or_create_user
...
async def show_referrals(callback: CallbackQuery, session: AsyncSession):
    ...
    from bot.database.queries import get_or_create_user, get_user
