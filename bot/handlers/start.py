...
async def send_main_menu(target, lang: str, edit: bool = False):
    ...
    kb = main_menu_kb(settings.WEBAPP_URL, lang=lang)
...
async def send_language_choice(message: Message):
    """Показывает выбор языка новому пользователю."""
    text = "Выберите язык / Choose language / اختر اللغة / 选择语言:"
...
async def cmd_start(message: Message, command: CommandObject, session: AsyncSession, state: FSMContext):
    ...
    # Новым — выбор языка, старым — сразу меню
    if is_new:
...
async def show_appeals(callback: CallbackQuery, session: AsyncSession):
    ...
    try:
        await callback.message.edit_caption(
            ...
            reply_markup=appeals_kb(lang=user.language)
        )
    except Exception:
